...
<div style='text-align: center; padding: 5px 0;'>
    ...
    <p style='color: #666; font-size: 14px; margin: 0 0 10px 0;'>NHS Trusts & Healthcare Organizations</p>
</div>
""", unsafe_allow_html=True)

# Custom CSS (keeping for compatibility)
st.markdown("""
<style>
    .login-header {
...
if st.session_state.logged_in:
    ...
    with col2:
        ...
        if st.session_state.get('show_nhs_password_reset'):
            ...
            if 'nhs_reset_step' not in st.session_state:
            ...
            if st.session_state.nhs_reset_step == 1:
                    ...
                    if st.button("📧 Send Reset Code", type="primary", use_container_width=True, key="nhs_send_code"):
                        if reset_email:
                            try:
                                ...
                                if success:
                                    st.success(message)
                                    st.session_state.nhs_reset_step = 2
                                    st.rerun()
                                else:
                                    st.error(message)
                            except:
                ...
                with col2:
                    if st.button("← Start Over", key="nhs_reset_restart"):
                        ...
                        st.rerun()
                with col3:
                    if st.button("Cancel", key="nhs_reset_cancel"):
                        st.session_state.show_nhs_password_reset = False
                        st.session_state.nhs_reset_step = 1
                        st.rerun()
