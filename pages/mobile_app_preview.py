...
with tab1:
    ...
    with col1:
        search_term = st.text_input("🔍 Search", key="search_mobile_feedback")
    with col2:
        records = read_all_records('mobile_feedback')
        if records:
            csv_data = export_to_csv(records)
            st.download_button("📥 Export CSV", csv_data, "mobile_feedback.csv", "text/csv")
    
    # Get records
    records = read_all_records('mobile_feedback')
    
    if search_term:
        records = search_records('mobile_feedback', search_term)
    ...
    if records:
        ...
        for idx, record in enumerate(records):
            with st.expander(f"Feedback #{idx+1}: {record.get('id', 'Unknown')[:20]}..."):
                st.json(record)
                ...
                with col2:
                    if st.button(f"🗑️ Delete", key=f"delete_{record['id']}"):
                        if delete_record('mobile_feedback', record['id']):
                            st.success("Deleted!")
                            st.rerun()
    else:
