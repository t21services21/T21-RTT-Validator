...
st.markdown("---")
st.markdown("## 💼 Transfer Management")
...
with tab1:
    ...
    with col1:
        search_term = st.text_input("🔍 Search", key="search_transfers")
    with col2:
        records = read_all_records('transfers')
        if records:
            csv_data = export_to_csv(records)
            st.download_button("📥 Export CSV", csv_data, "transfers.csv", "text/csv")
    
    # Get records
    records = read_all_records('transfers')
    
    if search_term:
        records = search_records('transfers', search_term)
    ...
    if records:
        ...
        for idx, record in enumerate(records):
            with st.expander(f"Transfer #{idx+1}: {record.get('id', 'Unknown')[:20]}..."):
                st.json(record)
                ...
                with col2:
                    if st.button(f"🗑️ Delete", key=f"delete_{record['id']}"):
                        if delete_record('transfers', record['id']):
                            st.success("Deleted!")
                            st.rerun()
    else:
