...
st.markdown("---")
st.markdown("## 💼 Dashboard Item Management")

tab1, tab2, tab3 = st.tabs(["📋 View All", "➕ Add New", "📊 Analytics"])

with tab1:
    st.subheader("📋 All Dashboard Items")
    
    # Search
    col1, col2 = st.columns([3, 1])
    with col1:
        search_term = st.text_input("🔍 Search", key="search_dashboard_data")
    with col2:
        records = read_all_records('dashboard_data')
        if records:
            csv_data = export_to_csv(records)
            st.download_button("📥 Export CSV", csv_data, "dashboard_data.csv", "text/csv")
    
    # Get records
    records = read_all_records('dashboard_data')
    
    if search_term:
        records = search_records('dashboard_data', search_term)
    ...
    if records:
        ...
        for idx, record in enumerate(records):
            with st.expander(f"Dashboard Item #{idx+1}: {record.get('id', 'Unknown')[:20]}..."):
                st.json(record)
                ...
                with col2:
                    if st.button(f"🗑️ Delete", key=f"delete_{record['id']}"):
                        if delete_record('dashboard_data', record['id']):
                            st.success("Deleted!")
                            st.rerun()
    else:
...
with tab2:
    st.subheader("➕ Add New Dashboard Item")
